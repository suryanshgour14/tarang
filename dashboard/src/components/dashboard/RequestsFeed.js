...
const PLACEHOLDER_IMAGE = 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=800&q=60';

function toFeedItem(report) {
  return {
    id: report.id,
    location: report.users?.name ? `Reported by ${report.users.name}` : 'Unknown reporter',
    description: report.description,
    image: report.media_urls?.[0] || PLACEHOLDER_IMAGE,
...
export default function RequestsFeed() {
  ...
  useEffect(() => {
    let cancelled = false;
    dashboardApi.getReports({ status: 'new', limit: 50 })
      .then((result) => {
        if (cancelled) return;
        if (result.data?.length) {
          setRequests(result.data.map(toFeedItem));
          setIsDemo(false);
        } else {
          setIsDemo(false);
          setRequests([]);
      ...
      .catch(() => {
        // Keep demo data if the API call fails (e.g. not signed in yet).
      });
  ...
  const handleAction = async (id, action) => {
    if (isDemo) {
      // Demo data has no backing report to update - just reflect it locally.
      if (action === 'reject') {
    ...
    const targetStatus = action === 'reject' ? 'rejected' : 'verified';
    try {
      await dashboardApi.verifyReport(id, targetStatus);
      setRequests(requests.filter(req => req.id !== id));
    } catch (err) {
  ...
  return (
    <div className="p-6">
      ...
      {!isDemo && requests.length === 0 && (
        <p className="text-slate-400 text-sm py-8 text-center">No pending reports right now.</p>
      )}
