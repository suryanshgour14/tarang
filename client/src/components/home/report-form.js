...
export default function ReportForm() {
  ...
  // API endpoints - the orchestrator (waterbody check + geotag + Supabase) and the
  // hazard-aggregation service, both configurable since they're separate deploys.
  const REPORT_ORCHESTRATOR_URL = process.env.NEXT_PUBLIC_REPORT_ORCHESTRATOR_URL || 'http://127.0.0.1:8002'
  const WATER_HAZARDS_SERVICE_URL = process.env.NEXT_PUBLIC_WATER_HAZARDS_URL || 'http://127.0.0.1:8003'
  const LIVE_CAPTURE_API = `${REPORT_ORCHESTRATOR_URL}/reports/submit/live_capture`
  const FILE_UPLOAD_API = `${REPORT_ORCHESTRATOR_URL}/reports/submit/file_upload`
  const WATER_HAZARDS_API = `${WATER_HAZARDS_SERVICE_URL}/fetch_water_hazards/`
